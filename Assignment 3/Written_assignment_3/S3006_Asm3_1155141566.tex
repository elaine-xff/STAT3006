...
\begin{flalign*}
    \theta_{jk} | - &\propto \prod_{i=1}^{1000} \prod_{k=1}^3 \Big[ \prod_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} \Big]^{I(Z_j = k)}\\
                    &\propto \prod_{i=1}^{1000} \Big[ \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} \Big]\\
    \text{Then it follows that:}&\\
    \theta_{jk} | - &\sim Bino(10j, \theta_{jk})
\end{flalign*}
For $Z$:
\begin{flalign*}
...
\begin{flalign*}
    ...
    \theta_{jk}^{(t+1)} & \sim Bino(10j , \theta_{jk}^{(t)}) \text{  for $j = 1, 2, 3$ and $k = 1, 2, 3$}\\
