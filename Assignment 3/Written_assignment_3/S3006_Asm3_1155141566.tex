...
Then using the same notation in the question:
\begin{flalign*}
    f(X, Z | \Pi, \Theta) &= \prod_{i=1}^{1000} \prod_{k=1}^3 P(Z_i = k | \Pi, \Theta) P(X_{ij}, j=1, 2, 3 | Z_j, \Pi, \Theta)\\
...
\begin{flalign*}
    P(\Pi, \Theta, Z | X) &\propto P(\Pi) P(\Theta) P(X, Z | \Pi, \Theta)\\
                          &\propto \sum_{k=1}^3 \pi_k^{\alpha_k - 1} \sum_{i=1}^{1000} \sum_{k=1}^3 \Big[ \pi_k \sum_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} \Big]^{I(Z_j = k)}\\
                          &\propto \sum_{k=1}^3 \pi_k^{\alpha_k - 1} \sum_{i=1}^{1000} \sum_{k=1}^3 \pi_k^{I(Z_i = k)} \sum_{i=1}^{1000} \sum_{k=1}^3 \sum_{j=1}^3 \binom{10j}{X_{ij}}^{I(Z_i = k)} \theta_{jk}^{X_{ij}I(Z_i = k)} [(1-\theta_{jk})^{10j - X_{ij}}]^{I(Z_i = k)}
\end{flalign*}
Then for each parameter:

For $\pi$:
\begin{flalign*}
    f(\Pi | \Theta, Z) &\propto \sum_{k=1}^3 \pi_k^{\alpha_k - 1} \sum_{i=1}^{1000} \sum_{k=1}^3 \pi_k^{I(Z_i = k)}\\
                       &\propto \sum_{k=1}^3 \pi_k^{\alpha_k - 1} \sum_{k=1}^3 [\pi_k]^{\sum_{i=1}^{1000} I(Z_i = k)}\\
                       &\propto \sum_{k=1}^3 \pi_k^{\alpha_k + \sum_{i=1}^{1000} I(Z_i = k) - 1}\\
...
\begin{flalign*}
    \theta_{jk} | - &\propto \prod_{i=1}^{1000} \prod_{k=1}^3 \Big[ \prod_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} \Big]^{I(Z_j = k)}\\
                    &\propto \prod_{i=1}^{1000} \Big[ \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} \Big]^{I(Z_j = k)}\\
    \text{Then it follows that:}&\\
    \theta_{jk} | - &\sim Bino(10j \sum_{i=1}^{1000} I(Z_i = k), \theta_{jk})
\end{flalign*}
For $Z$:
\begin{flalign*}
    Z_i | - &\propto \sum_{i=1}^{1000} \sum_{k=1}^3 \pi_k^{I(Z_i = k)} \sum_{i=1}^{1000} \sum_{k=1}^3 \sum_{j=1}^3 \binom{10j}{X_{ij}}^{I(Z_i = k)} \theta_{jk}^{X_{ij}I(Z_i = k)} [(1-\theta_{jk})^{10j - X_{ij}}]^{I(Z_i = k)}\\
            &\propto \sum_{k=1}^3 \pi_k^{I(Z_i = k)} \sum_{k=1}^3 \sum_{j=1}^3 \binom{10j}{X_{ij}}^{I(Z_i = k)} \theta_{jk}^{X_{ij}I(Z_i = k)} [(1-\theta_{jk})^{10j - X_{ij}}]^{I(Z_i = k)}\\
            &\propto \Biggl[ \sum_{k=1}^3 \pi_k \Big[  \sum_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}}\Big] \Biggr] ^{I(Z_i = k)}
\end{flalign*}
...
\begin{flalign*}
    ...
    \theta_{jk}^{(t+1)} & \sim Bino(10j \sum_{i=1}^{1000} I(Z_i^{(t)} = k), \theta_{jk}) \text{  for $j = 1, 2, 3$ and $k = 1, 2, 3$}\\
    P(Z_i^{(t+1)} = k | -) &= \frac{\pi_k^{(t+1)} \prod_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jk}^{X_{ij}} (1-\theta_{jk})^{10j - X_{ij}} }{\sum_{l=1}^3 \pi_l^{(t+1)} \prod_{j=1}^3 \binom{10j}{X_{ij}} \theta_{jl}^{X_{ij}} (1-\theta_{jl})^{10j - X_{ij}}}
\end{flalign*}
