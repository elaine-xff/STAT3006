...
p=array(NA,dim=c(N,4),dimnames=list(paste0("iter=",1:N),c("p1","p2","p3","p4")))
Y=array(NA,dim=c(N,4),dimnames=list(paste0("iter=",1:N),c("y11","y12","y22","y24")))
p[1,]=rDirichlet(c(2,2,2,2))
Y[1,]=c(15,32,15,31)

#Step 2: MH within Gibbs sampler
MH_y<- function(y0,p,low,upper){
  total=low+upper
  if(y0==low){
    y1=y0+1
  }else if(y0==upper){
    ...
    y1=sample(c(y0+1,y0-1),size=1,prob=c(1/2,1/2))
  }
  U=runif(1)
  ratio=factorial(y1)*factorial(total-y1)*(p[1]^y0)*(p[2]^(total-y0))/
    factorial(y0)/factorial(total-y0)/(p[1]^y1)/(p[2]^(total-y1))
  if(y1==low | y1==upper){
    ratio=2*ratio
  } else if(y0==low | y0==upper){
    ratio=0.5*ratio
  }
  ratio=min(1,ratio)
  if(U<ratio){
...
for(iter in 2:N){
  #sample p
  p[iter,]=rDirichlet(c(Y[iter-1,"y11"]+30,2+Y[iter-1,"y12"]+Y[iter-1,"y22"],
                   50,33+Y[iter-1,"y24"]))
  
  #sample y (MH step)
  Y[iter,"y12"]=MH_y(Y[iter-1,"y12"],p=p[iter,c(2,1)],low=15,upper=32)
  Y[iter,"y11"]=47-Y[iter,"y12"]
  Y[iter,"y22"]=MH_y(Y[iter-1,"y22"],p=p[iter,c(2,4)],low=15,upper=31)
  Y[iter,"y24"]=46-Y[iter,"y22"]
  
  if(iter %% 500==0) print(paste0("iter=",iter))
}


#Step 4: estimate lambda
B<- 5000
p_use<-p[(B+1):N,]
(p_mean=apply(p_use,2,mean))
