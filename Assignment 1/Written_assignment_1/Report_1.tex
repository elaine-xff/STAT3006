...
\begin{flalign*}
    \frac{\partial l}{\partial \pi_1}\bigg\rvert_{\hat{\pi_1}, \hat{\pi_2}} &= \sum_{i=1}^n \biggl[I(Z_i = 1)\frac{1}{\pi_1} - I(Z_i = 3)\frac{1}{1-\pi_1-\pi_2}\biggr]\bigg\rvert_{\hat{\pi_1}, \hat{\pi_2}} = 0\\
    \frac{\partial l}{\partial \pi_2}\bigg\rvert_{\hat{\pi_1}, \hat{\pi_2}} &= \sum_{i=1}^n \biggl[I(Z_i = 2)\frac{1}{\pi_2} - I(Z_i = 3)\frac{1}{1-\pi_1-\pi_2}\biggr]\bigg\rvert_{\hat{\pi_1}, \hat{\pi_2}} = 0\\
    \frac{\partial l}{\partial \mu_1}\bigg\rvert_{\hat{\mu_1}, \hat{\sigma_1^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 1) \frac{y_i - \mu_1}{\sigma_1^2}\biggr]\bigg\rvert_{\hat{\mu_1}, \hat{\sigma_1^2}} = 0\\
    \frac{\partial l}{\partial \mu_2}\bigg\rvert_{\hat{\mu_2}, \hat{\sigma_2^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 2) \frac{y_i - \mu_2}{\sigma_2^2}\biggr]\bigg\rvert_{\hat{\mu_2}, \hat{\sigma_2^2}} = 0\\
    \frac{\partial l}{\partial \mu_3}\bigg\rvert_{\hat{\mu_3}, \hat{\sigma_3^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 3) \frac{y_i - \mu_3}{\sigma_3^2}\biggr]\bigg\rvert_{\hat{\mu_3}, \hat{\sigma_3^2}} = 0\\
    \frac{\partial l}{\partial \sigma_1^2}\bigg\rvert_{\hat{\mu_1}, \hat{\sigma_1^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 1)\biggl(-\frac{1}{2}\frac{1}{\sigma_1^2} + \frac{(y_i - \mu_1)^2}{2(\sigma_1^2)^2}\biggr)\biggr]\bigg\rvert_{\hat{\mu_1}, \hat{\sigma_1^2}} = 0\\
    \frac{\partial l}{\partial \sigma_2^2}\bigg\rvert_{\hat{\mu_2}, \hat{\sigma_2^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 2)\biggl(-\frac{1}{2}\frac{1}{\sigma_2^2} + \frac{(y_i - \mu_2)^2}{2(\sigma_2^2)^2}\biggr)\biggr]\bigg\rvert_{\hat{\mu_2}, \hat{\sigma_2^2}} = 0\\
    \frac{\partial l}{\partial \sigma_3^2}\bigg\rvert_{\hat{\mu_3}, \hat{\sigma_3^2}} &= \sum_{i=1}^n \biggl[I(Z_i = 3)\biggl(-\frac{1}{2}\frac{1}{\sigma_3^2} + \frac{(y_i - \mu_3)^2}{2(\sigma_3^2)^2}\biggr)\biggr]\bigg\rvert_{\hat{\mu_3}, \hat{\sigma_3^2}} = 0
\end{flalign*}
Hence, we can conclude that:
\begin{flalign*}
    \hat{\pi_1} &= \frac{\sum_{i=1}^n I(Z_i = 1)(1 - \hat{\pi_2})}{\sum_{i=1}^n [I(Z_i = 1) + I(Z_i = 3)]}\\
    \hat{\pi_2} &= \frac{\sum_{i=1}^n I(Z_i = 2)(1 - \hat{\pi_1})}{\sum_{i=1}^n [I(Z_i = 2) + I(Z_i = 3)]}\\
    \hat{\mu_1} &= \frac{\sum_{i=1}^n I(Z_i = 1) y_i}{\sum_{i=1}^n I(Z_i = 1)}\\
    \hat{\mu_2} &= \frac{\sum_{i=1}^n I(Z_i = 2) y_i}{\sum_{i=1}^n I(Z_i = 2)}\\
    \hat{\mu_3} &= \frac{\sum_{i=1}^n I(Z_i = 3) y_i}{\sum_{i=1}^n I(Z_i = 3)}\\
    \hat{\sigma_1^2} &= \frac{\sum_{i=1}^n I(Z_i = 1) (y_i^2 - \hat{\mu_1^2} + 2\hat{\mu_1}y_i)}{\sum_{i=1}^n I(Z_i = 1)}\\
    \hat{\sigma_2^2} &= \frac{\sum_{i=1}^n I(Z_i = 2) (y_i^2 - \hat{\mu_2^2} + 2\hat{\mu_2}y_i)}{\sum_{i=1}^n I(Z_i = 2)}\\
    \hat{\sigma_3^2} &= \frac{\sum_{i=1}^n I(Z_i = 3) (y_i^2 - \hat{\mu_3^2} + 2\hat{\mu_3}y_i)}{\sum_{i=1}^n I(Z_i = 3)}
\end{flalign*}
Then given initial guess $\pi_1^{(0)}, \pi_2^{(0)}, \mu_1^{(0)}, \mu_2^{(0)}, \sigma_1^{(0)}, \sigma_2^{(0)}$, 
...
\begin{flalign*}
    Q(\Theta; \Theta^{(t)}) &= E_{\pi_1^{(t)}, \pi_2^{(t)}, \mu_1^{(t)}, \mu_2^{(t)}, \mu_3^{(t)}, {\sigma_1^2}^{(t)}, {\sigma_2^2}^{(t)}, {\sigma_3^2}^{(t)}} \bigl( l(\pi_1, \pi_2, \mu_1, \mu_2, \mu_3, \sigma_1, \sigma_2, \sigma_3 | \mathbf{Y}, \mathbf{Z}) | \mathbf{Y} \bigr)\\
                            &= \sum_{i=1}^n \Biggl[I(Z_i = 1) \biggl[\log \pi_1^{(t)} - \log \sqrt{2\pi} - \frac{1}{2}\log {\sigma_1^2}^{(t)} - \frac{(y_i - \mu_1^{(t)})^2}{2 {\sigma_1^2}^{(t)}}\biggr]\\
                            &+I(Z_i = 2) \biggl[\log \pi_2^{(t)} - \log \sqrt{2\pi} - \frac{1}{2}\log {\sigma_2^2}^{(t)} - \frac{(y_i - \mu_2^{(t)})^2}{2 {\sigma_2^2}^{(t)}}\biggr]\\
                            &+I(Z_i = 3) \biggl[\log (1-\pi_1^{(t)}-\pi_2^{(t)}) - \log \sqrt{2\pi} - \frac{1}{2}\log {\sigma_3^2}^{(t)} - \frac{(y_i - \mu_3^{(t)})^2}{2 {\sigma_3^2}^{(t)}}\biggr]\Biggr]
\end{flalign*}
